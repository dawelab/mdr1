...
inPath = '/scratch/gent/merged_maps/wt_endo_dev-mat_CG.gz'

#prefix for output files 
outPrefix = 'wt_endo_dev-mat_CG_3X'
...
EMRFile = open(outPrefix + '_EMRs.txt', 'w')
...
if coverageAve1 >= minCov:
	...
	if coverageAve2 >= minCov:
		...
		if countC >= minCountC:
			...
			methAve1 = sum(regionMeths1)/countC
			methAve2 = sum(regionMeths2)/countC
			regionLine = chr + '\t' + str(regionStart) + '\t' + str(regionStart + binSize - 1) + '\t' + str(methAve1) + '\t' + str(methAve2) + '\t' + str(coverageAve1) + '\t' + str(coverageAve2) + '\t' + str(countC) + '\n'
			EMRFile.write(regionLine) #write region details to eligible methylation regions file 
			EMRs += 1 #count number of eligible methylation regions
...
summaryFile.write('EMRs\t' + str(EMRs) + '\n')
